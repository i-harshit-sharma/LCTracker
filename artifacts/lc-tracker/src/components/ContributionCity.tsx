  ...
  return (
    <div className="w-full h-[500px] bg-slate-950 rounded-xl overflow-hidden border border-border/50 relative group">
      <div className="absolute top-4 left-4 z-10 pointer-events-none">
        <h3 className="text-sm font-semibold text-white/90">Contribution City</h3>
        <p className="text-xs text-white/50">3D Visualization of your LeetCode progress</p>
      </div>

      <Canvas shadows dpr={[1, 2]}>
        <PerspectiveCamera makeDefault position={[40, 40, 40]} fov={35} far={1000} />
        <OrbitControls 
          enablePan={true} 
          enableZoom={true} 
        ...
        <group position={[-weeks.length / 2, 0, -3.5]}>
          ...
          {/* Floor */}
          <mesh rotation={[-Math.PI / 2, 0, 0]} position={[weeks.length / 2 - 0.5, -0.01, 3]} receiveShadow>
