    ...
    <div className="w-full overflow-x-auto pb-2 scrollbar-hide">
      <div className="min-w-175">
        ...
        >
          {/* Month Labels */}
          {monthLabels.map((m, i) => (
            <text key={i} x={labelWidth + m.offset * (cellSize + gap)} y={10}>
              {m.label}
            </text>
          ))}

          {/* Day Labels */}
          <text x={0} y={labelHeight + 1 * (cellSize + gap) + 8}>
            Mon
          </text>
          <text x={0} y={labelHeight + 3 * (cellSize + gap) + 8}>
            Wed
          </text>
          <text x={0} y={labelHeight + 5 * (cellSize + gap) + 8}>
            Fri
          </text>
          ...
          <TooltipProvider delayDuration={0}>
            {weeks.map((week, weekIndex) => (
              <g
                key={weekIndex}
                transform={`translate(${labelWidth + weekIndex * (cellSize + gap)}, ${labelHeight})`}
              >
